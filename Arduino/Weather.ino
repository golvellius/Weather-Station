...
// V0.1 Le he quitado una línea para que lea bien la presion atmosférica
//      Funcion para calcular inHg  
// V0.2 Se va a usar nuevo sensor BME280
//      Se añade el sensor UV 
// V0.3 Se corrigue el paso del parametro de solarradiation en W/m2 .
...
#define ENABLE_UV   // Activamos el sensor UV solar .
...
// analog I/O pins
const byte WDIR = A0;
//const byte LIGHT = A1; usamos el sesnsor de luz para el uv
...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//*****************************************************************
const float LOCAL_ALT_METERS = 19.0; //SET FOR YOUR LOCAL ALTITUDE *
//*****************************************************************
...
uint8_t reportTime = 60; //how often to report the weather - can be updated from BBB
...
void calcWeather()
{
  ...
  //Calc coeficiente UV
#ifdef ENABLE_UV
  uv_lvl = uv_Intensidad()*10; // Paso de mw/cm2 a w/m2
  light_lvl = uv_lvl;
#endif
...
// Calculo la presion en hpa para enviar a wunther
  ...
  unsigned int tlight_lvl = (unsigned int)(light_lvl * 100);   // Este valor está en W/m2 y se pasa como solarradiation 
...
Serial.println(baroin);
Serial.println(tempf);
Serial.println(thumidity);
Serial.println(trainin);
...
float uv_Intensidad()
{
  ...
  int refLevel = averageAnalogRead(REFERENCE_3V3);
